...
const GlowCardWrapper: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  return (
    <div className="w-full relative flex items-center justify-center" onMouseMove={handleMouseMove}>
      ...
      >
        <div
          className="w-11/12 h-5/6 md:w-4/5 md:h-4/5 rounded-3xl blur-[3px]"
          style={{
            background: 'radial-gradient(300px circle at var(--glow-x, 50%) var(--glow-y, 50%), rgba(255,255,255,0.15), transparent 70%)',
...
const ProjectCard: React.FC<ProjectCardProps> = ({ item }) => {
  ...
  return (
    <div
      ...
      className="relative bg-gradient-to-br from-black via-gray-900 to-gray-800 rounded-3xl shadow-2xl overflow-hidden flex flex-col md:flex-row group w-full transition-all duration-500 ease-out hover:shadow-white/20"
      style={{
        '--mouse-x': '50%',
        '--mouse-y': '50%',
        border: isHovered 
        ...
        backgroundImage: isHovered 
          ? `linear-gradient(rgb(0,0,0), rgb(0,0,0)), radial-gradient(120px circle at ${mousePosition.x}px ${mousePosition.y}px, rgba(255, 255, 255, 0.9), rgba(255, 255, 255, 0.6) 25%, rgba(255, 255, 255, 0.3) 50%, rgba(209,213,219,0.2) 80%)`
          : 'none',
    ...
    >
      ...
      <div
        ...
        style={{
          background: 'radial-gradient(350px circle at var(--mouse-x, 50%) var(--mouse-y, 50%), rgba(255,255,255,0.3), rgba(255,255,255,0.2) 35%, rgba(255,255,255,0.1) 60%, transparent 80%)',
          boxShadow: isHovered ? '0 0 80px rgba(255, 255, 255, 0.15)' : '0 0 40px rgba(255,255,255,0.05)',
      ...
      <div className="relative z-20 flex flex-col md:flex-row w-full h-full">
        {/* Text section */}
        <div className="p-6 md:p-8 flex-1 flex flex-col justify-center bg-black backdrop-blur-sm rounded-b-3xl md:rounded-l-3xl md:rounded-br-none">
          <h2 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-extrabold mb-4 pl-6 py-2 relative flex items-center transition-all duration-300">
            <span className="absolute left-0 top-1/2 -translate-y-1/2 h-8 md:h-10 w-1.5 md:w-2 rounded-full bg-gradient-to-b from-white via-gray-300 to-gray-600 shadow-lg shadow-white/30"></span>
            <span className="relative z-10 px-3 py-2 bg-black/20 rounded-xl shadow-lg text-white hover:bg-gray-800/30 hover:shadow-white/20 transition-all duration-300 backdrop-blur-sm border border-gray-500/20">
              {item.title}
            </span>
          </h2>
          <p className="text-gray-200 text-sm md:text-base leading-relaxed font-medium drop-shadow-sm group-hover:text-white transition-colors duration-300">
            {item.description}
          </p>
        </div>
        
        {/* Image section */}
        <div className="w-full md:w-80 lg:w-96 h-60 md:h-auto bg-black flex items-center justify-center overflow-hidden rounded-t-3xl md:rounded-t-none md:rounded-r-3xl ">
          <div className="w-full h-full p-2 md:p-3">
            <img
              src={item.image}
              alt={item.title}
              className="object-cover w-full h-full rounded-2xl md:rounded-2xl transition-all duration-500 ease-out hover:scale-105 shadow-lg border border-gray-600/50"
              style={{ 
                minHeight: '220px', 
                maxHeight: '300px',
              ...
              onError={(e) => {
                ...
                target.parentElement!.innerHTML = `
                  <div class="w-full h-full bg-gradient-to-br from-black to-gray-700 rounded-2xl flex items-center justify-center">
