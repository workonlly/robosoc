...
const Achievements: React.FC = () => {
  ...
  return (
    <div className="achievements min-h-screen py-6 sm:py-10 lg:py-16 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-6xl mx-auto relative">
        ...
        <div className="space-y-8 sm:space-y-16 md:space-y-20 lg:space-y-24 relative">
          <div className="absolute left-1/2 transform -translate-x-1/2 w-1 top-0 bottom-0 bg-gradient-to-b from-white/60 via-gray-400 to-white/60 rounded-full shadow-lg shadow-white/20 hidden sm:block"></div>
          
          {achievements.map((achievement, index) => (
            <div key={index} className={`relative flex items-center ${index % 2 === 0 ? 'sm:flex-row' : 'sm:flex-row-reverse'} flex-col sm:flex-row group`}>
              
              <div className="absolute left-1/2 transform -translate-x-1/2 z-20 hidden sm:block">
                <div className="w-6 h-6 bg-white rounded-full shadow-lg shadow-white/30 group-hover:scale-125 transition-transform duration-300">
              ...
              <div className={`w-full sm:w-full lg:w-5/12 ${index % 2 === 0 ? 'lg:pr-16 sm:text-right' : 'lg:pl-16 sm:text-left'} text-center sm:text-left`}>
                <div className="relative bg-black/60 backdrop-blur-xl rounded-2xl sm:rounded-3xl p-4 sm:p-6 md:p-8 lg:p-10 border border-white/10 shadow-2xl shadow-black/50 hover:bg-black/70 hover:border-white/30 hover:shadow-white/10 transition-all duration-500 hover:scale-105 group-hover:translate-y-2">
                  
                  <div className={`mb-4 sm:mb-6 ${index % 2 === 0 ? 'sm:text-right' : 'sm:text-left'} text-center sm:text-left`}>
                    <div className="inline-block bg-white/10 backdrop-blur-sm border-2 border-white/30 text-white font-bold text-lg sm:text-xl md:text-2xl px-4 sm:px-6 py-2 sm:py-3 rounded-lg sm:rounded-xl shadow-lg shadow-white/10 group-hover:scale-105 hover:bg-white/20 hover:border-white/40 transition-all duration-300">
                      {achievement.year}
                    </div>
                  </div>
